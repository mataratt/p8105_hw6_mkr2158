...
- My modeling process derives from previous research and literature on
  ...
  - `babysex`: selected since male infants generally may have higher
    average birth weights compared to female infants
  - `frace`: selected since a father’s genetic code may influence the
    growth which can influence birth weight
  - `gaweek`: selected since there is a general positive association
    between gestational week and birth week
  - `malform`: selected as birth defects may lead to lower birth weights
  - `momage`: selected as extreme ages (very young vs. very old) may
    lead to variations in risk of extreme low/high birth weights
  - `mrace`: selected since a mother’s genetic code may influence growth
    which influence birth weight
  - `ppbmi`: selected because pre-pregnancy BMI is directly correlated
    with birth weight. Low pre-pregnancy BMI may lead to lower birth
    weight and vice versa
  - `smoken`: selected because smoking causes a dose-response effect
    where higher number of cigarettes may lead to lower birth weight
...
![](p8105_hw6_mkr2158_files/figure-gfm/unnamed-chunk-6-1.png)<!-- -->

#### Comparison with Model with Length at Birth and Gestational Age

``` r
blength_ga_model =
  lm(bwt ~ blength + gaweeks, data = birthweight)

tidy(blength_ga_model) |> 
  knitr::kable()
```
...
#### Comparison with Model with Head Circumference, Length, Sex, and All Interactions
...
birthweight_cv =
  ...
  mutate(
    ...
    model_one = map(train, ~ lm(bwt ~ babysex + frace + gaweeks + malform +
                                momage + mrace + ppbmi + smoken, data = .x)),
    model_two = map(train, ~ lm(bwt ~ blength + gaweeks, data = .x)),
    model_three = map(train, ~ lm(bwt ~ bhead * blength * babysex, data = .x)),
    model_one_rmse = map2_dbl(
      model_one, test, \(mod, df) rmse(model = mod, data = df)
    ),
    model_two_rmse = map2_dbl(
      model_two, test, \(mod, df) rmse(model = mod, data = df)
    ),
    model_three_rmse = map2_dbl(
      model_three, test, \(mod, df) rmse(model = mod, data = df)
    )
  )
...
birthweight_cv |> 
  ...
  labs(
    title = expression(bold("Figure 4. Cross-Validated RMSE for All Birth Weight Models")),
    x = "Model",
    y = "RMSE",
    fill = "Model"
  ) +
...
![](p8105_hw6_mkr2158_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->

- Model 1: My model with predictors: babysex, frace, gaweeks, malform,
  momage, mrace, ppbmi, and smoken
- Model 2: with predictors Length at Birth and Gestational Age
- Model 3: with predictors Head Circumference, Length, Sex, and All
  Interactions
- **Conclusion**: Model 1 (my proposed model) generated the highest
  RMSE, centered around 450, compared to the other two models. Model 2
  performed substantially better than Model 1. However, Model 3,
  containing head circumference, length, sex, and interactions, provides
  the best prediction for birth weight and offers the best accuracy.
